...
Dependabot 보안 알림(CVE-2026-34601)에 따라 `@xmldom/xmldom` 취약점을 패치했습니다.

| 패키지 | 이전 버전 | 변경 버전 | 해결된 취약점 |
|--------|----------|----------|-------------|
| @xmldom/xmldom | 0.8.10 | 0.9.9 | XML injection via unsafe CDATA serialization (High) |

`resolutions`를 통해 `plist` 하위 의존성인 `@xmldom/xmldom`을 `^0.9.9`로 강제 업그레이드했습니다.
